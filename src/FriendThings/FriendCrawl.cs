...
public class FriendCrawl
{
    ...
    {
        orig(self);
        if (self.player.slugcatStats.name != Solace.FriendName || Solace.PoleCrawl() != true) return;
    ...
    public static bool SlugcatHand_EngageInMovement(On.SlugcatHand.orig_EngageInMovement orig, SlugcatHand self)
    {
        ...
        if (player.slugcatStats.name != Solace.FriendName || Solace.PoleCrawl() != true) return orig(self);
